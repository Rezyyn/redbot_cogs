...
class AprilAI(commands.Cog):
    ...
    async def join_voice(self, ctx):
        ...
        # Ensure we’re in a guild and user is in a voice channel
        if not ctx.guild:
            return await ctx.send("❌ This command only works in servers!")
        if not ctx.author.voice or not ctx.author.voice.channel:
            return await ctx.send("❌ You need to be in a voice channel!")

        # Try to invoke the Audio cog’s summon command
        audio_cog = self.bot.get_cog("Audio")
        if audio_cog:
            # This calls [p]summon under the hood
            await ctx.invoke(self.bot.get_command("summon"))
            return

        # Fallback (should never hit if Audio is installed)
    ...
    async def process_query(self, ctx, input_text):
        ...
        async with ctx.typing():
            try:
                ...
                # send text if either not in VC or user wants text
    ...
    async def ttskey(self, ctx, key: str):
        await self.config.tts_key.set(key); await ctx.tick()
    @commands.command()
    @commands.is_owner()
    async def voiceid(self, ctx, vid: str):
        await self.config.voice_id.set(vid); await ctx.tick()
    @commands.command()
    @commands.is_owner()
    async def togglevoice(self, ctx):
        cur = await self.config.tts_enabled()
        await self.config.tts_enabled.set(not cur)
        await ctx.send(f"🔊 Voice {'Enabled' if not cur else 'Disabled'}")
    @commands.command()
    @commands.is_owner()
    async def toggletxt(self, ctx):
        cur = await self.config.text_response_when_voice()
        await self.config.text_response_when_voice.set(not cur)
        await ctx.send(f"📝 Text w/Voice {'Enabled' if not cur else 'Disabled'}")

    @commands.command()
    async def leave(self, ctx):
        """Leave voice via Audio cog."""
        audio_cog = self.bot.get_cog("Audio")
        if audio_cog:
            await ctx.invoke(self.bot.get_command("leave"))
        else:
            await ctx.send("❌ Audio cog not found or not connected.")
